	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyinleliliao`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussyinleliliao`, this.ruleForm);
				}else{
					await this.$api.add(`discussyinleliliao`, this.ruleForm);
				}
				uni.setStorageSync('discussyinleliliaoCleanType',true);
